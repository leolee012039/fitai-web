...
export const useNutritionStore = create()(persist((set, get) => ({
    logs: {},
    water: {},
    goals: {
        calorie_goal: 2000,
    ...
    fetchLogsForDate: async (date) => {
        try {
            const { data } = await apiClient.get('/api/nutrition/logs', { params: { date } });
            const mapped = data.map((d) => ({ ...d, log_id: d.log_id ?? d.id ?? '', name_zh: d.food_name, id: d.log_id }));
            set({ logs: { ...get().logs, [date]: mapped } });
        }
        catch {
    ...
    analyzePhotoWithAI: async (mimeType, base64) => {
        try {
            const { data } = await apiClient.post('/api/nutrition/analyze-photo', {
                mime_type: mimeType,
                data_base64: base64,
            });
